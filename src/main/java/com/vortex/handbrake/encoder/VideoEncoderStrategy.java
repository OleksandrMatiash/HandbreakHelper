...
public class VideoEncoderStrategy implements EncoderStrategy {
    ...
    public File encode(File srcFile, Consumer<String> logConsumer, Consumer<BigDecimal> progressConsumer) {
        try {
            ...
            process = Runtime.getRuntime().exec(FilesHelper.getFileFullPath("/Handbrake/HandbrakeCLI.exe")
...
//                + " -w 1280"
                            + " --crop 0:0:0:0"
                            + " --loose-anamorphic"
                            + " --modulus 2 -e x264 -q 30 --vfr -a 1 -E av_aac -6 stereo -R Auto " // 22 - excellent quality, 30 - low quality
                            + "-B 160 -D 0 --gain 0 --audio-fallback ac3 --encoder-preset=veryfast  --encoder-level=\"4.0\"  --encoder-profile=main  --verbose=1"
            );
            ...
            while (processIsAlive || thereIsSomethingToRead) {
                ...
                if (readerOut.ready()) {
                    ...
                    if (progressConsumer != null) {
                        ...
                        if (matcher.matches()) {
                            progressConsumer.accept(new BigDecimal(matcher.group(1)));
                        }
                    }
                }
